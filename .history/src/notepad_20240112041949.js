/*
  maybe one day
    - https://learn.microsoft.com/en-us/azure/developer/github/deploy-with-visual-studio
*/

/*
  consider handling page scrollbar layout shift in a more profound way
    - https://dev.to/rashidshamloo/preventing-the-layout-shift-caused-by-scrollbars-2flp
    - write to index.css
*/

/* 
  can next determine how to render checkboxes and not relevant state and number of options selected
    - do we want to handle non-relevance when it comes to the filtered data too?
    
  also need all (& maybe `all relevant`) buttons in dropdowns
    
  also need to record state of measures to sum up, fields to group by, & regression type
  
  also need to have pivot field defined somewhere for each data file
*/

/*
  consider chad's settings & template data viz app idea
*/
