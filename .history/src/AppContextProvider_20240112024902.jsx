...
const useProvideGlobally = () => {
  ...
  const onBeforeEnd = useCallback((json, setResult) => {
    ...
    setColumnFilters((previousColFilters) => {
      const root = relevantColFilters;

      Object.entries(previousColFilters).forEach(([field, { checklist }]) => {
        if (!(field in root)) {
          root[field] = {
            checklist: Object.fromEntries(Object.entries(checklist)),
            relevant: false,
          };
        }
      });
