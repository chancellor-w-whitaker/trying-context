...
const useProvideGlobally = () => {
  ...
  const onBeforeEnd = useCallback((data, setResult) => {
    const fieldToColObject = {};

    data.forEach((row) => {
      Object.keys(row).forEach((field) => {
        if (!(field in fieldToColObject)) {
          fieldToColObject[field] = { values: new Set(), types: {} };
        }

        const { values, types } = fieldToColObject[field];
    ...
    const columns = Object.entries(fieldToColObject).map(
      ([field, { values, types }]) => ({
    ...
    const fieldToValueSet = Object.fromEntries(
      columns.map(({ values, field }) => [field, values])
    );

    console.log(fieldToValueList);

    console.log(columns);

    setResult({ fieldToValueList, fieldToValueSet, data });
  }, []);
