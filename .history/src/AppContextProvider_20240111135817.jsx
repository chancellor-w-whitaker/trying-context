...
const useProvideGlobally = () => {
  ...
  const onBeforeEnd = useCallback((json, setResult) => {
    const fieldToDerivedInfo = {};

    json.forEach((row) => {
      Object.keys(row).forEach((field) => {
        if (!(field in fieldToDerivedInfo)) {
          fieldToDerivedInfo[field] = { values: new Set(), types: {} };
        }

        const { values, types } = fieldToDerivedInfo[field];
    ...
    const columns = Object.entries(fieldToDerivedInfo).map(
      ([field, { values, types }]) => ({
